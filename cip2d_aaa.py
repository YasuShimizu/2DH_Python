...
def u_cal(un,gux,guy,u,v_up,fn,gxn,gyn,nx,ny,dx,dy,dt):
    ...
    for i in np.arange(1,nx+1):
        for j in np.arange(1,ny+1):
            ...
            gux[i,j]=gxn[i,j]-(gxo*(u[i+1,j]-u[i-1,j])+gyo*(v_up[i+1,j]-v_up[i-1,j]))*.5*dt/dx
            guy[i,j]=gxn[i,j]-(gxo*(u[i,j+1]-u[i,j-1])+gyo*(v_up[i,j+1]-v_up[i,j-1]))*.5*dt/dy
    return un,gux,guy

def v_cal(vn,gvx,gvy,u_vp,v,fn,gxn,gyn,nx,ny,dx,dy,dt):
    for i in np.arange(1,nx+1):
    ...
    for i in np.arange(1,nx+1):
        for j in np.arange(1,ny):
            ...
            gvx[i,j]=gxn[i,j]-(gxo*(u_vp[i+1,j]-u_vp[i-1,j])+gyo*(v[i+1,j]-v[i-1,j]))*0.5*dt/dx
            gvy[i,j]=gyn[i,j]-(gxo*(u_vp[i,j+1]-u_vp[i,j-1])+gyo*(v[i,j+1]-v[i,j-1]))*0.5*dt/dy
    return vn,gvx,gvy